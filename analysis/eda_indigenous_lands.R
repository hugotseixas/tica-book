...
il |>
  summarise(
    il_area = sum(il_area),
    .by = c("terrai_cod", "creation_year")
  ) |>
  drop_na(terrai_cod) |>
  mutate(percentage = il_area / sum(il_area) * 100) |>
...
timeseries_plot <- il %>%
  ...
  select(!geometry) %>%
  summarise(
    il_area = as.numeric(sum(il_area, na.rm = TRUE)),
    .by = c("year", "region_name")
  ) %>%
  arrange(year) %>%
  mutate(
    il_area = cumsum(il_area),
    il_area = il_area * 0.0001,
    .by = "region_name"
  ) %>%
  ggplot(
    aes(
...
ggsave(
  filename = "./figs/indigenous_lands_timeseries.png",
...
# Save map plot
ggsave(
  filename = "./figs/indigenous_lands_map.png",
  plot = map_plot,
  device = ragg::agg_png,
  width = 15,
  height = 8,
